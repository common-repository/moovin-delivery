...
(function ($) {
  ...
  function GMapContext(domElement, options) {
    _map = new google.maps.Map(domElement, options);
  ...
  var GmUtility = {
      ...
      gmapContext.geodecoder.geocode(
        ...
        function (results, status) {
          if (status == google.maps.GeocoderStatus.OK && results.length > 0) {
            var address = GmUtility.addressByFormat(
              results,
              gmapContext.settings.addressFormat
            );
            gmapContext.locationName = address.formatted_address;
            gmapContext.addressComponents =
              GmUtility.address_component_from_google_geocode(
    ...
    address_component_from_google_geocode: function (address_components) {
      ...
      result.addressLine1 = [result.streetNumber, result.streetName]
        .join(" ")
        .trim();
      result.addressLine2 = "";
      return result;
  ...
  function updateInputValues(inputBinding, gmapContext) {
    ...
    var currentLocation = GmUtility.locationFromLatLng(
      gmapContext.marker.position
    );
  ...
  function updateMap(gmapContext, $target, options) {
    ...
    GmUtility.setPosition(
      gmapContext,
      new google.maps.LatLng(
        gmapContext.settings.location.latitude,
        gmapContext.settings.location.longitude
      ),
      function (context) {
        setupInputListenersInput(
          gmapContext.settings.inputBinding,
          gmapContext
        );
    ...
    if (typeof options == "string") {
      ...
      switch (options) {
        ...
        case "map":
          ...
          if (params == undefined) {
            // Getter
            var locationObj = GmUtility.locationFromLatLng(
              gmapContext.location
            );
